...
%Adam: the function mean will simply average each column and return a row
%vector. Since one of the values in column 2 is a NaN, then that average
%cannot be computed. The meannonan will replace any NaN with an empty
%bracket, but also reoder the array in to a single row array. This will
%then yield the average of the entire row.
